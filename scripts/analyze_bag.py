...
Options:
  ...
  --course TEXT          REQUIRED. Course identifier (e.g. "corridor-right",
                         "room-loop"). Metrics depend heavily on the course,
                         so this must be recorded for valid comparisons.
...
def analyze(bag_path: str, target_distance_mm: int, label: str, course: str) -> dict:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="TEXT",
        help="Course identifier (e.g. 'corridor-right', 'room-loop'). Required.",
    )
    ...
    metrics = analyze(bag_path, args.target_distance, args.label, args.course)
