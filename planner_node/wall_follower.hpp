...
class WallFollower {
 ...
 private:
  ...
  void UpdateFollowing(const WallFollowerSensors& sensors) {
    ...
    const float effective_speed{
        is_corner ? static_cast<float>(config_.base_speed_mm_s) * config_.corner_speed_ratio
                  : static_cast<float>(config_.base_speed_mm_s)};

    auto correction{static_cast<int16_t>(std::clamp(
        config_.kp * error + config_.kd * d_error + config_.ki * i_error_,
