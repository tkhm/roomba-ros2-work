#ifndef PLANNER_NODE_WALL_FOLLOWER_HPP_
#define PLANNER_NODE_WALL_FOLLOWER_HPP_

#include <algorithm>
#include <cmath>
#include <cstdint>

// Wall-following state machine and PID-controller for Roomba 600.
//
...
//   FOLLOWING  — Wall detected; PID-control maintains target wall distance.
//               Corner detection (|d_error| > corner_detect_threshold) reduces
//               speed by corner_speed_ratio to avoid overshooting bends.
...
struct WallFollowerConfig {
  ...
  // Proportional gain for wall distance PID-controller.
  // correction = kp * error + kd * (error - prev_error) + ki * i_error
  ...
  // Integral gain.  Eliminates steady-state offset that P+D cannot remove.
  // Start at 0 (disabled).  Increase slowly — values above ~0.01 may cause windup.
  // Anti-windup: i_error is clamped so that ki * i_error <= max_correction_mm_s.
  float ki{0.0F};

  // Emergency avoidance threshold [wall_signal units].
  // When wall_signal exceeds this value the PID-controller is bypassed and a
  ...
  // Corner detection: when |d_error| exceeds this threshold, the robot is
  // assumed to be entering a corner and reduces speed by corner_speed_ratio.
  // Prevents overshooting bends at higher base speeds.
  // Units: wall_signal delta per update cycle.
  uint16_t corner_detect_threshold{50};

  // Speed multiplier applied when a corner is detected (0.0–1.0).
  // 0.6 = 60% of base_speed_mm_s.
  float corner_speed_ratio{0.6F};
};
 ...
 private:
  ...
  void UpdateSearching(const WallFollowerSensors& sensors) {
    ...
    if (sensors.wall_signal >= config_.wall_detect_threshold) {
      ...
      i_error_ = 0.0F;     // reset I term on entry
  ...
  void UpdateFollowing(const WallFollowerSensors& sensors) {
    ...
    // PID-control: proportional + derivative + integral
    ...
    // I term eliminates steady-state offset that P+D cannot remove.
    ...
    // Integral with anti-windup: clamp so ki * i_error <= max_correction_mm_s.
    i_error_ += error;
    if (config_.ki > 0.0F) {
      const float windup_limit{static_cast<float>(config_.max_correction_mm_s) / config_.ki};
      i_error_ = std::clamp(i_error_, -windup_limit, windup_limit);
    }

    // Corner detection: large d_error indicates a bend approaching.
    // Reduce speed to prevent overshooting.
    const bool is_corner{std::abs(d_error) > static_cast<float>(config_.corner_detect_threshold)};
    const float effective_speed{is_corner ? static_cast<float>(config_.base_speed_mm_s) *
                                                config_.corner_speed_ratio
                                          : static_cast<float>(config_.base_speed_mm_s)};

    auto correction{static_cast<int16_t>(
        std::clamp(config_.kp * error + config_.kd * d_error + config_.ki * i_error_,
                   static_cast<float>(-config_.max_correction_mm_s),
                   static_cast<float>(config_.max_correction_mm_s)))};
    speeds_.left_mm_s =
        std::clamp(static_cast<int16_t>(static_cast<int16_t>(effective_speed) + correction),
  ...
  void UpdateRecovering(int32_t elapsed_ms) {
    ...
    if (recovery_elapsed_ms_ < config_.recovery_backup_ms) {
      ...
      // Recovery complete: return to FOLLOWING so PID-control can re-acquire wall
      speeds_ = {0, 0};
...
#endif  // PLANNER_NODE_WALL_FOLLOWER_HPP_
